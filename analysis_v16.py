...
"""
花岗岩单轴压缩试验 - 综合分析 v16
干扰识别：时变振幅条带 × 时间窗（仅安静期）
  核心改进（参照 CH4 无干扰通道效果）：
    ① 爆发段（is_burst=True）完全不过滤 → 破坏期所有hits全部保留（同CH4）
    ② 安静期时间窗延长至 1000ms → 捕捉持续超过500ms的干扰条带
    ③ 拟合范围：安静期去掉末端200s（避免爆发前真实AE污染峰值估计）
    ④ 外推改为恒值（取安静期有效拟合段最后若干箱均值），不用斜率
"""
...
# ═══════════════════════════════════════════════════════════════════════════
print("\n"+"="*60)
print("干扰识别 v16（安静期：时变条带×时间窗  爆发段：不过滤）...")
...
print(f"  爆发段（{t_burst_start:.0f}s起）完全不过滤，共 {is_burst.sum()} hits 免过滤")
...
# 三条件同时满足 → 干扰（爆发段 is_burst=True 时不删）
contam    = in_tw & on_stripe & ~no_filter & ~is_burst
...
print(f"\n  [统计]  时间窗内总计: {int(in_tw.sum())}  安静期时间窗: {int((in_tw&~is_burst).sum())}")
print(f"          安静期条带内（干扰）: {n_contam} ({pct:.1f}%)  "
      f"爆发段全部保留: {int(is_burst.sum())}")
for ch in range(1,7):
    cm=ae_ch==ch
    note=' ← 不过滤' if ch in CH_NO_FILTER else ''
    burst_ch = int((is_burst & cm).sum())
    print(f"    CH{ch}: 删 {int((contam&cm).sum()):>5}  留 {int((~contam&cm).sum()):>5}"
          f"  (其中爆发段保留 {burst_ch}){note}")
...
plt.tight_layout()
out1=os.path.join(RESULT_DIR,'v16_01_条带拟合诊断.png')
...
fig2.suptitle(
    f'声发射振幅 全段干扰对比 v16 (0-{T_MAX:.0f}s)\n'
    f'安静期条带×时间窗 | 爆发段不过滤  干扰 {n_contam} ({pct:.1f}%)  真实AE {n_clean} ({100-pct:.1f}%)',
    fontsize=10,fontweight='bold')
...
for ch in range(1,7):
    ...
    if row==0: axL.set_title(f'原始 ({n_hits} hits)  红线=干扰中心±条带  橙色=不过滤区',fontsize=11,pad=8)
...
out2=os.path.join(RESULT_DIR,'v16_02_全段干扰对比图.png')
...
fig3.suptitle('超声波与声发射综合分析 v16\nVp(AIC) + AE(安静期条带×时间窗，爆发段不过滤)',fontsize=12,fontweight='bold')
...
ax3b.set_ylabel('AE振幅 (dB)'); ax3b.set_title(f'声发射振幅 (v16 去干扰后，爆发段全保留)')
...
vfail(axes3,t_fail)
plt.tight_layout()
out3=os.path.join(RESULT_DIR,'v16_03_综合分析.png'); fig3.savefig(out3); plt.close(fig3)
print(f"图3已保存: {out3}")
...
plt.tight_layout()
out4=os.path.join(RESULT_DIR,'v16_04_P波速度.png'); fig4.savefig(out4); plt.close(fig4)
print(f"图4已保存: {out4}")
...
if len(evts)>0 and 'x' in evts.columns:
    ...
    plt.tight_layout()
    out5=os.path.join(RESULT_DIR,'v16_05_AE事件空间分布.png')
    fig5.savefig(out5); plt.close(fig5)
    print(f"图5已保存: {out5}")

# ─── CSV ─────────────────────────────────────────────────────────────────
out_vp=os.path.join(RESULT_DIR,'v16_Vp_AIC.csv')
...
out_ae=os.path.join(RESULT_DIR,'v16_AE_clean.csv');       ae_clean.to_csv(out_ae,  index=False)
out_ct=os.path.join(RESULT_DIR,'v16_AE_contaminated.csv'); ae_contam.to_csv(out_ct, index=False)

print("\n"+"="*60)
print("[超声波 Vp]")
print(f"  t_cal={sys_delay:.3f}us  初始Vp={np.nanmean(us_Vp_aic[valid_aic][:10]):.3f}km/s  峰值Vp={np.nanmax(us_Vp_aic):.3f}km/s")
print(f"\n[声发射干扰识别 v16 — 安静期条带×时间窗，爆发段不过滤]")
