...
US_MASK_POST_BURST_FRAC = 0.90  # 爆发段扩展窗 = US周期 × 此分数
...
CH_NO_FILTER   = {4}    # 无干扰通道：跳过振幅带+时间窗过滤，全部保留
DENSITY_WINDOW = 2.0    # 密度检测时间半窗 (s)：同通道带内 hits 稠密 → 干扰
DENSITY_THRESH = 4      # 同通道带内局部 hits 计数阈值（含两侧窗口，排除自身）
...
# ═══════════════════════════════════════════════════════════════════════════
print("\n"+"="*60)
print("干扰识别 v11（各通道独立 BND_FRAC + 动态扩展时间窗 + CH5上限兜底）...")
...
# 密度检测：逐通道计算带内 hits 局部稠密度，稠密 → 周期性干扰，孤立 → 真实 AE
is_dense=np.zeros(n_hits,dtype=bool)
for ch in range(1,7):
    if ch in CH_NO_FILTER: continue
    ch_band_mask=in_ch_band&(ae_ch==ch)
    band_t=ae_t[ch_band_mask]; band_idx=np.where(ch_band_mask)[0]
    if len(band_t)==0: continue
    idx_r=np.searchsorted(band_t, band_t+DENSITY_WINDOW, side='right')
    idx_l=np.searchsorted(band_t, band_t-DENSITY_WINDOW, side='left')
    local_n=idx_r-idx_l-1   # 排除自身
    is_dense[band_idx[local_n>=DENSITY_THRESH]]=True
print(f"  密集干扰带 hits: {is_dense.sum()} / 带内总 {in_ch_band.sum()} "
      f"  孤立保留: {int((in_ch_band&~is_dense).sum())}")
...
# ── v11 干扰判定：密度法（非爆发段）+ 扩展时间窗（爆发段）──────────────────
no_filter = np.isin(ae_ch, list(CH_NO_FILTER))   # CH_NO_FILTER 通道一律保留

# 非爆发段：带内密集簇 → 干扰；带内孤立点 → 真实 AE
contam_band_quiet = is_dense & ~is_burst
# 爆发段：带内 hits 仍用扩展时间窗（爆发期间密度高，密度法可能误判真实 AE）
contam_band_burst = in_ch_band & is_burst & contam_tw_ext
# 基础时间窗：带外 hits（US 脉冲 500ms 内）
contam_tw_out     = contam_tw & ~in_ch_band
# 合并（CH_NO_FILTER 通道一律不删）
contam = (contam_band_quiet | contam_band_burst | contam_tw_out) & ~no_filter
...
n_band_dense_quiet= int(contam_band_quiet.sum())
n_band_burst_del  = int(contam_band_burst.sum())
n_band_isolated   = int((in_ch_band & ~is_dense & ~no_filter).sum())  # 孤立点（全段保留）
n_band_burst_kept = int((in_ch_band & is_burst & ~contam_tw_ext).sum())

print(f"\n  时间窗去除(带外):              {n_tw_only} hits ({100.*n_tw_only/n_hits:.1f}%)")
print(f"  振幅带去除(非爆发密集簇):      {n_band_dense_quiet} hits ({100.*n_band_dense_quiet/n_hits:.1f}%)")
print(f"  振幅带去除(爆发扩展窗):        {n_band_burst_del} hits ({100.*n_band_burst_del/n_hits:.1f}%)")
print(f"  振幅带孤立点保留(非爆发):      {int((in_ch_band&~is_dense&~is_burst&~no_filter).sum())} hits")
...
ax_d.bar(ac2,h_bq,width=1.8,color='tomato',  alpha=0.8,bottom=h_tw,label=f'振幅带干扰(非爆发密集簇) ({n_band_dense_quiet})')
...
plt.tight_layout()
out1=os.path.join(RESULT_DIR,'v11_01_通道振幅带与爆发诊断.png')
...
fig2.suptitle(
    f'声发射振幅 全段干扰对比 v11 (0-{T_MAX:.0f}s)\n'
...
out2=os.path.join(RESULT_DIR,'v11_02_全段干扰对比图.png')
...
ax3b.set_title(f'声发射振幅 (v11 去干扰后，爆发段~{t_burst_start:.0f}s起)')
...
plt.tight_layout()
out3=os.path.join(RESULT_DIR,'v11_03_综合分析.png')
...
plt.tight_layout()
out4=os.path.join(RESULT_DIR,'v11_04_P波速度.png')
...
if len(evts)>0 and 'x' in evts.columns:
    ...
    plt.tight_layout()
    out5=os.path.join(RESULT_DIR,'v11_05_AE事件空间分布.png')
    fig5.savefig(out5); plt.close(fig5)
    print(f"图5已保存: {out5}")

# ─── CSV ─────────────────────────────────────────────────────────────────
out_vp=os.path.join(RESULT_DIR,'v11_Vp_AIC.csv')
...
out_ae=os.path.join(RESULT_DIR,'v11_AE_clean.csv'); ae_clean.to_csv(out_ae,index=False)
out_ct=os.path.join(RESULT_DIR,'v11_AE_contaminated.csv'); ae_contam.to_csv(out_ct,index=False)
...
print(f"  振幅带去除(非爆发密集簇):   {n_band_dense_quiet}")
print(f"  振幅带去除(爆发扩展窗):     {n_band_burst_del}")
print(f"  振幅带孤立保留(非爆发):     {int((in_ch_band&~is_dense&~is_burst&~no_filter).sum())}")
