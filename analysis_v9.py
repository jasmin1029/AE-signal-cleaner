...
# ═══════════════════════════════════════════════════════════════════════════
print("\n"+"="*60)
print("干扰识别 v9（各通道独立 BND_FRAC + 动态扩展时间窗 + CH5上限兜底）...")
...
# 爆发段中振幅带 hits：用扩展时间窗过滤周期性干扰（覆盖约90% US周期）
...
print(f"  振幅带保留(爆发真实AE):    {n_band_burst_kept} hits ({100.*n_band_burst_kept/n_hits:.1f}%)")
...
plt.tight_layout()
out1=os.path.join(RESULT_DIR,'v9_01_通道振幅带与爆发诊断.png')
...
fig2.suptitle(
    f'声发射振幅 全段干扰对比 v9 (0-{T_MAX:.0f}s)\n'
...
out2=os.path.join(RESULT_DIR,'v9_02_全段干扰对比图.png')
...
ax3b.set_title(f'声发射振幅 (v9 去干扰后，爆发段~{t_burst_start:.0f}s起)')
...
plt.tight_layout()
out3=os.path.join(RESULT_DIR,'v9_03_综合分析.png')
...
plt.tight_layout()
out4=os.path.join(RESULT_DIR,'v9_04_P波速度.png')
...
if len(evts)>0 and 'x' in evts.columns:
    ...
    plt.tight_layout()
    out5=os.path.join(RESULT_DIR,'v9_05_AE事件空间分布.png')
    fig5.savefig(out5); plt.close(fig5)
    print(f"图5已保存: {out5}")

# ─── CSV ─────────────────────────────────────────────────────────────────
out_vp=os.path.join(RESULT_DIR,'v9_Vp_AIC.csv')
...
out_ae=os.path.join(RESULT_DIR,'v9_AE_clean.csv'); ae_clean.to_csv(out_ae,index=False)
out_ct=os.path.join(RESULT_DIR,'v9_AE_contaminated.csv'); ae_contam.to_csv(out_ct,index=False)
