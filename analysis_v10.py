...
# ═══════════════════════════════════════════════════════════════════════════
print("\n"+"="*60)
print("干扰识别 v10（各通道独立 BND_FRAC + 动态扩展时间窗 + CH5上限兜底）...")
...
# ── v10 改进：振幅带 hits 全段统一用扩展时间窗判定 ────────────────────────
# 振幅带内，距任意 US 脉冲 >2.81s（≈10% 周期）的孤立点 → 保留为真实 AE
# 之前版本非爆发段对振幅带 hits 一律删除，错误地去掉了这部分真实 AE
contam_band_del = in_ch_band & contam_tw_ext          # 带内 + 扩展窗 → 干扰
contam_tw_out   = contam_tw   & ~in_ch_band            # 带外 + 基础窗 → 干扰
contam   = contam_band_del | contam_tw_out
...
n_band_del       = int(contam_band_del.sum())
n_band_kept_free = int((in_ch_band & ~contam_tw_ext).sum())   # 带内孤立点（保留）
n_band_quiet     = int((in_ch_band & ~is_burst & contam_tw_ext).sum())  # 非爆发段删除
n_band_burst_del = int((in_ch_band &  is_burst & contam_tw_ext).sum())  # 爆发段删除
n_band_burst_kept= int((in_ch_band &  is_burst & ~contam_tw_ext).sum()) # 爆发段孤立点

print(f"\n  时间窗去除(带外):          {n_tw_only} hits ({100.*n_tw_only/n_hits:.1f}%)")
print(f"  振幅带去除(扩展窗内):      {n_band_del} hits ({100.*n_band_del/n_hits:.1f}%)")
print(f"  振幅带保留(孤立点全段):    {n_band_kept_free} hits ({100.*n_band_kept_free/n_hits:.1f}%)")
...
h_bq, _=np.histogram(ae_amp[in_ch_band&~is_burst&contam_tw_ext], bins=ae2)
h_bb, _=np.histogram(ae_amp[in_ch_band& is_burst&contam_tw_ext], bins=ae2)
h_cl, _=np.histogram(ae_amp[~contam],                bins=ae2)
ax_d.bar(ac2,h_tw,width=1.8,color='silver',  alpha=0.8,label=f'时间窗干扰 ({n_tw_only})')
ax_d.bar(ac2,h_bq,width=1.8,color='tomato',  alpha=0.8,bottom=h_tw,label=f'振幅带干扰(非爆发扩展窗) ({n_band_quiet})')
...
plt.tight_layout()
out1=os.path.join(RESULT_DIR,'v10_01_通道振幅带与爆发诊断.png')
...
fig2.suptitle(
    f'声发射振幅 全段干扰对比 v10 (0-{T_MAX:.0f}s)\n'
...
out2=os.path.join(RESULT_DIR,'v10_02_全段干扰对比图.png')
...
ax3b.set_title(f'声发射振幅 (v10 去干扰后，爆发段~{t_burst_start:.0f}s起)')
...
plt.tight_layout()
out3=os.path.join(RESULT_DIR,'v10_03_综合分析.png')
...
plt.tight_layout()
out4=os.path.join(RESULT_DIR,'v10_04_P波速度.png')
...
if len(evts)>0 and 'x' in evts.columns:
    ...
    plt.tight_layout()
    out5=os.path.join(RESULT_DIR,'v10_05_AE事件空间分布.png')
    fig5.savefig(out5); plt.close(fig5)
    print(f"图5已保存: {out5}")

# ─── CSV ─────────────────────────────────────────────────────────────────
out_vp=os.path.join(RESULT_DIR,'v10_Vp_AIC.csv')
...
out_ae=os.path.join(RESULT_DIR,'v10_AE_clean.csv'); ae_clean.to_csv(out_ae,index=False)
out_ct=os.path.join(RESULT_DIR,'v10_AE_contaminated.csv'); ae_contam.to_csv(out_ct,index=False)
...
print(f"  振幅带去除(扩展窗内全段):   {n_band_del}")
print(f"  振幅带保留(孤立点全段):     {n_band_kept_free}")
print(f"    其中 非爆发段孤立点:      {int((in_ch_band&~is_burst&~contam_tw_ext).sum())}")
print(f"    其中 爆发段孤立点:        {n_band_burst_kept}")
print(f"  总干扰: {n_contam} ({pct:.1f}%)  真实AE: {n_clean} ({100-pct:.1f}%)")
print(f"\n[输出目录] {RESULT_DIR}")
