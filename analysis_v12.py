...
花岗岩单轴压缩试验 - 综合分析 v12
改进（相对 v7）：
  1. 各通道独立 BND_FRAC（越小→振幅带越宽），按用户反馈分通道调整：
       CH1 0.10  CH2 0.10  CH3 0.15  CH4 0.30(不变)  CH5 0.10  CH6 0.10
  2. US_MASK_POST_BURST 改为动态：= period_approx × 0.90（≈90% US周期），
     从 1.5s 扩展至约 2.8s，覆盖 CH2/CH3/CH6 近破坏段剩余周期性干扰
  3. CH5 上限兜底：若拟合上限 < 全局上限，强制取全局上限（补偿样本少的问题）
"""
...
# US_MASK_POST_BURST 将在 §3 根据 period_approx 动态计算
US_MASK_POST_BURST_FRAC = 0.50  # 爆发段核心带扩展窗 = US周期 × 此分数
                                # 0.50 ≈ 1.57s（覆盖~50%周期），原0.90≈2.81s覆盖率太高导致近破坏真实AE被误删
US_MASK_POST_BURST_MIN  = 1.50  # 爆发段扩展窗下限 (s)

T_EARLY     = 200.0   # 用前 200s 拟合干扰振幅带
BND_FRAC    = 0.30    # 全局/回退带截断高度
# 各通道独立截断高度（越小 → 振幅带越宽）
CH_BND_FRAC = {
    1: 0.10,   # CH1：上限偏小，扩宽
    2: 0.10,   # CH2：上下边界均需扩宽，覆盖初始段及近破坏段
    3: 0.15,   # CH3：上限略扩宽，减少近破坏干扰
    4: 0.30,   # CH4：处理效果好，维持原值
    5: 0.10,   # CH5：样本少 + 上限明显偏小，扩宽；另加上限兜底
    6: 0.10,   # CH6：扩宽覆盖"上波浪"次级干扰
}
MIN_FIT_N   = 15      # 各通道拟合最少样本数，不足则回退全局带

# 各通道振幅带手动边界微调（叠加在自动拟合结果上，单位 dB）
#   正值 = 扩大；负值 = 收窄；0 = 不调整
CH_ADJ_LOW  = {1: 0.0, 2: -6.0, 3: 0.0, 4: 0.0, 5: 0.0, 6:  0.0}
CH_ADJ_HIGH = {1:+6.0, 2:  0.0, 3: 0.0, 4: 0.0, 5: 0.0, 6: +6.0}

CH_NO_FILTER   = {4}    # 无干扰通道：保留全部 hits，跳过所有过滤
# ── 双层振幅带策略（v12 改进）──────────────────────────────────────────────
#   核心带 = 各通道自动拟合结果（调整前）→ 干扰密集区，非爆发段全部删除
#   扩展带 = 手动微调扩大的区域（核心带之外）→ 孤立真实 AE 多在此处
#           距任何 US 脉冲 ≤500ms 的删除，>500ms 的视为真实 AE 保留

RATE_BIN    = 5.0     # 速率分箱宽度 (s)
BURST_FACTOR= 3.0     # 高振幅 hits 速率 > 早期基准 × 此倍数 → 爆发段
MIN_BURST_RATE = 0.05 # 绝对最小触发速率 (hits/s)
...
def fit_amp_band(amps, ae_fine, ac_fine, bnd_frac, label=''):
    """从振幅样本拟合干扰带，返回 (low, high, peak, smoothed_hist)"""
...
# ═══════════════════════════════════════════════════════════════════════════
print("\n"+"="*60)
print("干扰识别 v12（各通道独立 BND_FRAC + 动态扩展时间窗 + CH5上限兜底）...")

# ── US 周期 → 动态计算爆发段扩展时间窗 ────────────────────────────────────
period_approx=float(np.median(np.diff(us_ts)))
US_MASK_POST_BURST = max(period_approx * US_MASK_POST_BURST_FRAC, US_MASK_POST_BURST_MIN)
print(f"  US扫查周期(中位): {period_approx:.2f}s  爆发段扩展窗: {US_MASK_POST_BURST:.2f}s")
...
print(f"  全局干扰振幅带(BND_FRAC={BND_FRAC}): [{g_low:.1f},{g_high:.1f}] dB (峰值 {g_peak:.1f} dB)")

# 各通道单独拟合
ch_low={}; ch_high={}; ch_peak={}; ch_hs={}
ch_core_low={}; ch_core_high={}   # 自动拟合边界（调整前），供双层带逻辑使用
print(f"  {'CH':>3}  {'bfrac':>6}  {'峰值':>6}  {'带范围':>16}  {'样本数':>6}  备注")
for ch in range(1,7):
    mask_ch=ae_ch==ch
    ...
    if len(fit_ch)<MIN_FIT_N:
        ...
        ch_core_low[ch]=g_low; ch_core_high[ch]=g_high   # 回退时核心带=全局带
        ch_hs[ch]=None
        print(f"  CH{ch}  {'---':>6}  {g_peak:>5.1f}  [{g_low:>5.1f},{g_high:>5.1f}]  n={len(fit_ch):>3}  回退全局")
        continue
    bfrac=CH_BND_FRAC.get(ch, BND_FRAC)
    lo,hi,pk,hs_ch=fit_amp_band(fit_ch,ae_fine,ac_fine,bfrac,f'CH{ch}')
    note=''
    # CH5：上限不低于全局上限（补偿样本不足）
    if ch==5 and hi<g_high:
        hi=g_high
        note='上限兜底至全局'
    # 存储核心带边界（调整前）供双层振幅带逻辑使用
    ch_core_low[ch]=lo; ch_core_high[ch]=hi
    # 叠加手动微调（扩展带边界）
    lo = lo + CH_ADJ_LOW.get(ch, 0.0)
    hi = hi + CH_ADJ_HIGH.get(ch, 0.0)
    adj_str=''
    if CH_ADJ_LOW.get(ch,0)!=0 or CH_ADJ_HIGH.get(ch,0)!=0:
        adj_str=f' adj({CH_ADJ_LOW[ch]:+.0f},{CH_ADJ_HIGH[ch]:+.0f})'
        note=(note+adj_str).strip()
    ch_low[ch],ch_high[ch],ch_peak[ch],ch_hs[ch]=lo,hi,pk,hs_ch
    print(f"  CH{ch}  {bfrac:>6.2f}  {pk:>5.1f}  [{lo:>5.1f},{hi:>5.1f}]  n={len(fit_ch):>3}  {note}")

# 各通道振幅带掩膜（CH_NO_FILTER 通道不纳入）
in_ch_band=np.zeros(n_hits,dtype=bool)
for ch in range(1,7):
    if ch in CH_NO_FILTER: continue
    cm=ae_ch==ch
    in_ch_band|=cm&(ae_amp>=ch_low[ch])&(ae_amp<=ch_high[ch])

global_band_max=max(ch_high.values())

# 核心带掩膜（各通道自动拟合结果，不含手动扩展部分；CH_NO_FILTER 通道不纳入）
in_core_band=np.zeros(n_hits,dtype=bool)
for ch in range(1,7):
    if ch in CH_NO_FILTER: continue
    cm=ae_ch==ch
    in_core_band|=cm&(ae_amp>=ch_core_low[ch])&(ae_amp<=ch_core_high[ch])
# 扩展带 = 手动调整后带范围 - 核心带（仅 CH1/CH2/CH6 存在扩展段）
in_edge_band = in_ch_band & ~in_core_band

# ── Step-2  用"高振幅 hits（> 全局带上边界+5 dB）"检测爆发段起点 ──────────
AMP_ABOVE=global_band_max+5.0
...
# ── Step-3  组合干扰判定 ───────────────────────────────────────────────────
# 扩展时间窗掩膜（动态 US_MASK_POST_BURST ≈ 90% US周期）
contam_tw_ext = ((dt_from_us>=-US_MASK_PRE)&(dt_from_us<=US_MASK_POST_BURST)) | \
                ((dt_to_next>=0)&(dt_to_next<=US_MASK_PRE))

# ── 双层振幅带判定（v12）──────────────────────────────────────────────────
# 核心带（自动拟合区域，干扰密集）：非爆发段全部删除
# 扩展带（手动调整扩大区域）：  仅删除基础时间窗（500ms）内的点；
#   距任何 US 脉冲 >500ms 的孤立点 → 视为真实 AE 保留
no_filter = np.isin(ae_ch, list(CH_NO_FILTER))   # CH_NO_FILTER 通道一律保留

# 非爆发段
contam_core_quiet = in_core_band & ~is_burst                    # 核心带全删
contam_edge_quiet = in_edge_band & ~is_burst & contam_tw        # 扩展带仅删时间窗内
# 爆发段（近破坏）：核心带用扩展窗，扩展带仅用基础窗
# — 近破坏时真实AE大量出现，扩展带内孤立点应尽量保留
contam_core_burst = in_core_band & is_burst & contam_tw_ext     # 核心带：扩展窗
contam_edge_burst = in_edge_band & is_burst & contam_tw         # 扩展带：基础窗
# 基础时间窗：带外 hits（US 脉冲 500ms 内）
contam_tw_out     = contam_tw & ~in_ch_band
# 合并（CH_NO_FILTER 通道一律不删）
contam = (contam_core_quiet | contam_edge_quiet | contam_core_burst | contam_edge_burst | contam_tw_out) & ~no_filter

# 扩展带孤立点（保留的真实 AE）
contam_band_quiet = contam_core_quiet | contam_edge_quiet   # 用于直方图绘制（非爆发）
contam_band_burst = contam_core_burst | contam_edge_burst   # 用于直方图绘制（爆发）
n_edge_isolated   = int((in_edge_band & ~is_burst & ~contam_tw & ~no_filter).sum())
print(f"  扩展带孤立点(非爆发，距US脉冲>500ms): {n_edge_isolated}  (视为真实AE保留)")
...
n_tw_only          = int(contam_tw_out.sum())
n_core_quiet_del   = int(contam_core_quiet.sum())
n_edge_quiet_del   = int(contam_edge_quiet.sum())
n_band_quiet_del   = n_core_quiet_del + n_edge_quiet_del
n_core_burst_del   = int(contam_core_burst.sum())
n_edge_burst_del   = int(contam_edge_burst.sum())
n_band_burst_del   = n_core_burst_del + n_edge_burst_del
n_band_burst_kept  = int((in_ch_band & is_burst & ~contam_core_burst & ~contam_edge_burst).sum())
n_isolated_nonburst= n_edge_isolated

print(f"\n  时间窗去除(带外):              {n_tw_only} hits ({100.*n_tw_only/n_hits:.1f}%)")
print(f"  振幅带去除(核心带非爆发):      {n_core_quiet_del} hits ({100.*n_core_quiet_del/n_hits:.1f}%)")
print(f"  振幅带去除(扩展带时间窗内):    {n_edge_quiet_del} hits ({100.*n_edge_quiet_del/n_hits:.1f}%)")
print(f"  振幅带去除(核心带爆发扩展窗):  {n_core_burst_del} hits ({100.*n_core_burst_del/n_hits:.1f}%)")
print(f"  振幅带去除(扩展带爆发基础窗):  {n_edge_burst_del} hits ({100.*n_edge_burst_del/n_hits:.1f}%)")
print(f"  扩展带孤立点保留(非爆发):      {n_edge_isolated} hits")
print(f"  振幅带孤立点保留(爆发):        {n_band_burst_kept} hits")
...
fig1.suptitle(
    f'v12 干扰识别诊断  各通道振幅带 + 高振幅速率爆发检测\n'
    f'CH_BND_FRAC: CH1=0.10 CH2=0.10 CH3=0.15 CH4=0.30 CH5=0.10(+兜底) CH6=0.10  '
    f'扩展窗={US_MASK_POST_BURST:.2f}s (≈{US_MASK_POST_BURST/period_approx*100:.0f}%周期)',
...
for ch in range(1,7):
    ...
    if len(fit_ch)>0:
        ...
        if ch_hs[ch] is not None:
            ax.plot(ac_fine,ch_hs[ch]/ch_hs[ch].max()*max(h.max(),1),
...
h_tw, _=np.histogram(ae_amp[contam_tw_out],                       bins=ae2)
h_bq, _=np.histogram(ae_amp[contam_band_quiet],                   bins=ae2)
h_bb, _=np.histogram(ae_amp[contam_band_burst],                   bins=ae2)
h_cl, _=np.histogram(ae_amp[~contam],                             bins=ae2)
ax_d.bar(ac2,h_tw,width=1.8,color='silver',  alpha=0.8,label=f'时间窗干扰 ({n_tw_only})')
ax_d.bar(ac2,h_bq,width=1.8,color='tomato',  alpha=0.8,bottom=h_tw,label=f'振幅带干扰(非爆发) ({n_band_quiet_del})')
...
plt.tight_layout()
out1=os.path.join(RESULT_DIR,'v12_01_通道振幅带与爆发诊断.png')
...
fig2.suptitle(
    f'声发射振幅 全段干扰对比 v12 (0-{T_MAX:.0f}s)\n'
    f'各通道独立振幅带(独立BND_FRAC) + 高振幅速率检测爆发(起始~{t_burst_start:.0f}s)'
    f' + 爆发段扩展窗({US_MASK_POST_BURST:.2f}s ≈{US_MASK_POST_BURST/period_approx*100:.0f}%周期)\n'
...
out2=os.path.join(RESULT_DIR,'v12_02_全段干扰对比图.png')
...
ax3b.set_title(f'声发射振幅 (v12 去干扰后，爆发段~{t_burst_start:.0f}s起)')
...
plt.tight_layout()
out3=os.path.join(RESULT_DIR,'v12_03_综合分析.png')
...
plt.tight_layout()
out4=os.path.join(RESULT_DIR,'v12_04_P波速度.png')
...
if len(evts)>0 and 'x' in evts.columns:
    ...
    plt.tight_layout()
    out5=os.path.join(RESULT_DIR,'v12_05_AE事件空间分布.png')
    fig5.savefig(out5); plt.close(fig5)
    print(f"图5已保存: {out5}")

# ─── CSV ─────────────────────────────────────────────────────────────────
out_vp=os.path.join(RESULT_DIR,'v12_Vp_AIC.csv')
...
out_ae=os.path.join(RESULT_DIR,'v12_AE_clean.csv'); ae_clean.to_csv(out_ae,index=False)
out_ct=os.path.join(RESULT_DIR,'v12_AE_contaminated.csv'); ae_contam.to_csv(out_ct,index=False)
...
print(f"\n[声发射干扰识别 v12]")
print(f"  US周期={period_approx:.2f}s  爆发段扩展窗={US_MASK_POST_BURST:.2f}s ({US_MASK_POST_BURST/period_approx*100:.0f}%周期)")
for ch in range(1,7):
    bfrac=CH_BND_FRAC.get(ch,BND_FRAC)
    note=' (CH5上限兜底)' if ch==5 else ''
    print(f"  CH{ch}: [{ch_low[ch]:.1f},{ch_high[ch]:.1f}] dB (峰值{ch_peak[ch]:.1f}dB  bfrac={bfrac}){note}")
print(f"  爆发段起始: ~{t_burst_start:.0f}s  (高振幅>{AMP_ABOVE:.0f}dB 速率>{burst_thresh_ab:.4f}hits/s)")
print(f"  时间窗去除(带外):             {n_tw_only}")
print(f"  振幅带去除(核心带非爆发):     {n_core_quiet_del}")
print(f"  振幅带去除(扩展带时间窗):     {n_edge_quiet_del}")
print(f"  振幅带去除(核心带爆发扩展窗): {n_core_burst_del}")
print(f"  振幅带去除(扩展带爆发基础窗): {n_edge_burst_del}")
print(f"  扩展带孤立保留(非爆发):       {n_edge_isolated}")
print(f"  振幅带孤立保留(爆发):         {n_band_burst_kept}")
print(f"  总干扰: {n_contam} ({pct:.1f}%)  真实AE: {n_clean} ({100-pct:.1f}%)")
print(f"\n[输出目录] {RESULT_DIR}")
