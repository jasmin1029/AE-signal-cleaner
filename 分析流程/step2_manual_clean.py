...
# -*- coding: utf-8 -*-
"""
step2_manual_clean.py — 手动清理交互工具
=========================================
读取 step1 输出的 AE_clean.csv，通过交互式图形界面手动删除残留干扰点，
最终将结果保存至 结果/step2_手动清理/。
...
操作说明:
  ...
  Z  / [撤  销]             → 撤销上一步操作（可多次）
  Esc / [清除选择]          → 取消当前橙色选中
  S  / [导出结果]           → 写 CSV + 生成统计图 PNG

运行方式:
  python step2_manual_clean.py
  （需先完成 step1_auto_filter.py）
"""
...
from config import (
    STEP1_AE_CLEAN,
    STEP2_DIR, STEP2_AE_CLEAN, STEP2_AE_DELETED,
    AE_HITS as AE_RAW_TXT,
    CH_COLORS,
    SCATTER_S, SCATTER_A, SAVE_DPI,
)
...
def parse_ae_hits(path: str) -> pd.DataFrame:
    """解析仪器原始 AE hits 文本文件（用于对比图的原始信号来源）。"""
    rows = []
...
class ManualCleaner:
    ...
    def __init__(self, data: pd.DataFrame,
                 out_dir: str = STEP2_DIR,
                 out_clean: str = STEP2_AE_CLEAN,
                 out_deleted: str = STEP2_AE_DELETED):
        self.data       = data.reset_index(drop=True)
        self.out_dir    = out_dir
        self.out_clean  = out_clean
        self.out_deleted= out_deleted
    ...
    def _build_ui(self):
        ...
        try:
            self.fig.canvas.manager.set_window_title('AE 手动清理工具 — Step 2')
        except Exception:
            pass
    ...
    def _do_export(self):
        ...
        ae_final.to_csv(self.out_clean,   index=False)
        ae_removed.to_csv(self.out_deleted, index=False)

        n_f  = len(ae_final)
        n_d  = len(ae_removed)
        pct  = 100.*n_d/self.n if self.n > 0 else 0.
        print(f"\n[导出]  保留 {n_f}  手动删除 {n_d} ({pct:.1f}%)")
        print(f"  {self.out_clean}")
        print(f"  {self.out_deleted}")

        self._status_txt.set_text('生成图中...')
        self.fig.canvas.draw_idle()
        ...
        try:
            ...
            mb.showinfo('导出完成',
                        ...
                        f'CSV: {os.path.basename(self.out_clean)}\n'
    ...
    def _gen_figure(self, ae_final: pd.DataFrame,
        ...
        try:
            fig2.suptitle(
                f'声发射 手动清理后最终结果  (step1 自动滤波 + step2 手动清理)\n'
            ...
            # ── 6 通道散点图（左：删/留叠加；右：仅最终保留）────────
            for ch in range(1, 7):
                row = ch - 1
                d_d = ae_removed[ae_removed['CH'] == ch]
                ...
                axL.text(0.01, 0.97, f'保留 {len(d_k)}  删 {len(d_d)}',
                         transform=axL.transAxes, fontsize=7, va='top', color='#333')
                if row == 0:
                    axL.set_title('step2 输入（灰=手动删除  彩=保留）', fontsize=10, pad=6)
            ...
            for ch in range(1, 7):
                ...
                if len(t_ch):
                    ax_c.plot(t_ch, c_ch, color=CH_COLORS[ch],
                              lw=0.8, alpha=0.7, zorder=2, label=f'CH{ch}')
            ...
            ax_c.set_title('累计振铃计数  (灰虚=清理前，彩=各通道，黑=清理后合计)',
                           fontsize=10, pad=4)
            ax_c.legend(ncol=8, fontsize=7, loc='upper left', framealpha=0.7)
            ...
            ax_e.plot(ae_after['Time'].values, cum_ener_after,
                      color='black', lw=1.6, zorder=3, label='清理后合计')
            ...
            ax_e.set_title('累计绝对能量  对数纵轴  (灰虚=清理前，彩=各通道，黑=清理后合计)',
                           fontsize=10, pad=4)
            ax_e.legend(ncol=8, fontsize=7, loc='upper left', framealpha=0.7)
            ax_e.grid(True, alpha=0.2, linewidth=0.4, which='both')

            out_png = os.path.join(self.out_dir, 'step2_清理结果.png')
            fig2.savefig(out_png, dpi=SAVE_DPI, bbox_inches='tight')
            return out_png
        finally:
            plt.close(fig2)
            plt.ion()
    ...
    def _gen_comparison_figure(self, ae_final: pd.DataFrame) -> str:
        ...
        右列：最终保留（step1 自动滤波 + step2 手动清理后）
        """
        ...
        try:
            ...
            for ch in range(1, 7):
                ...
                cln_ch  = ae_clean[ae_clean['CH'] == ch]
                n_r, n_c = len(raw_ch), len(cln_ch)
                ...
                axR.scatter(cln_ch['Time'], cln_ch['AMP'],
                            s=SZ, alpha=AL, color=CH_COLORS[ch],
                            zorder=2, rasterized=True)
                axR.set_xlim(0, xmax); axR.set_ylim(ylo, yhi)
                axR.yaxis.set_major_locator(MultipleLocator(20))
                ...
                if n_r > 0:
                    axR.text(0.01, 0.97,
                             f'{n_c} hits  (去除 {n_r-n_c}, {100.*(n_r-n_c)/n_r:.1f}%)',
                             transform=axR.transAxes, fontsize=7, va='top', color='#333')
                if row == 0:
            ...
            out_cmp = os.path.join(self.out_dir, 'step2_原始vs最终对比.png')
            fig3.savefig(out_cmp, dpi=SAVE_DPI, bbox_inches='tight')
            return out_cmp
        finally:
            plt.close(fig3)
            plt.ion()
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='AE 手动清理工具')
    parser.add_argument('--raw',   action='store_true',
                        help='直接读取仪器原始 TXT（跳过 step1）')
    parser.add_argument('--input', default=None,
                        help='指定输入 TXT 文件路径（自动启用 --raw 模式）')
    parser.add_argument('--outdir', default=None,
                        help='指定输出目录（默认 STEP2_DIR）')
    ...
    print("=" * 60)
    print(f"Step 2 — 手动清理交互工具  (后端: {_GUI_BACKEND})")
    print("=" * 60)
    ...
    # 确定输出 CSV 路径（保持在 out_dir 下，文件名与 STEP2 一致）
    out_clean   = os.path.join(out_dir, 'AE_final_clean.csv')
    out_deleted = os.path.join(out_dir, 'AE_manual_deleted.csv')

    if args.raw:
        input_path = args.input if args.input else AE_RAW_TXT
        if not os.path.exists(input_path):
            print(f"\n错误: 找不到输入文件:\n  {input_path}")
            sys.exit(1)
        print(f"[--raw 模式]  读取: {input_path}")
        data = parse_ae_hits(input_path)
        src_label = f'仪器原始数据 ({len(data)} hits)'
    else:
        if not os.path.exists(STEP1_AE_CLEAN):
            print(f"\n错误: 找不到 step1 输出文件:\n  {STEP1_AE_CLEAN}")
            print("请先运行 step1_auto_filter.py，或使用 --raw/--input 参数")
            sys.exit(1)
        data = pd.read_csv(STEP1_AE_CLEAN)
        src_label = f'step1 清理结果 ({len(data)} hits)'

    print(f"加载 {src_label}  通道: {sorted(data['CH'].unique())}")
    ...
    cleaner = ManualCleaner(data, out_dir=out_dir,
                            out_clean=out_clean, out_deleted=out_deleted)
    plt.show()
