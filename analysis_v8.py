...
# ═══════════════════════════════════════════════════════════════════════════
print("\n"+"="*60)
print("干扰识别 v8（各通道独立 BND_FRAC + 动态扩展时间窗 + CH5上限兜底）...")
...
plt.tight_layout()
out1=os.path.join(RESULT_DIR,'v8_01_通道振幅带与爆发诊断.png')
...
fig2.suptitle(
    f'声发射振幅 全段干扰对比 v8 (0-{T_MAX:.0f}s)\n'
...
out2=os.path.join(RESULT_DIR,'v8_02_全段干扰对比图.png')
...
ax3b.set_title(f'声发射振幅 (v8 去干扰后，爆发段~{t_burst_start:.0f}s起)')
...
plt.tight_layout()
out3=os.path.join(RESULT_DIR,'v8_03_综合分析.png')
...
plt.tight_layout()
out4=os.path.join(RESULT_DIR,'v8_04_P波速度.png')
...
if len(evts)>0 and 'x' in evts.columns:
    ...
    plt.tight_layout()
    out5=os.path.join(RESULT_DIR,'v8_05_AE事件空间分布.png')
    fig5.savefig(out5); plt.close(fig5)
    print(f"图5已保存: {out5}")

# ─── CSV ─────────────────────────────────────────────────────────────────
out_vp=os.path.join(RESULT_DIR,'v8_Vp_AIC.csv')
...
out_ae=os.path.join(RESULT_DIR,'v8_AE_clean.csv'); ae_clean.to_csv(out_ae,index=False)
out_ct=os.path.join(RESULT_DIR,'v8_AE_contaminated.csv'); ae_contam.to_csv(out_ct,index=False)
